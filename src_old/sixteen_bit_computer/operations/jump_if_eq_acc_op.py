...
Sets PC (jumps) to a constant if the module or constant passed as an
argument is equal to the value in ACC.
...
def generate_microcode_templates():
    ...
    Returns:
        list(DataTemplate): DataTemplates for all the JUMP_IF_EQ_ACC
        microcode.
    """
...
def generate_operation_templates(signature):
    """
    Create the DataTemplates to define a JUMP_IF_EQ_ACC with the given
    args.
...
def generate_nonconditional_steps(signature):
    ...
    if signature[0]["value_type"] == "constant":
        ...
        gen_flags_step_1.extend(ALU_CONTROL_FLAGS["COMPARE_LTE_GT_EQ"])
        ...
        gen_flags_step_0.extend(ALU_CONTROL_FLAGS["COMPARE_LTE_GT_EQ"])
        control_steps.append(gen_flags_step_0)
...
def generate_instruction_byte_bitdefs(signature):
    ...
    if signature[0]["value_type"] == "constant":
        # Original slot (00111111) used by JUMP_IF_NOT_UNDERFLOW_FLAG
        src = SRC_REGISTERS["ACC"]
    else:
        src = SRC_REGISTERS[signature[0]["value"]]
...
def parse_line(line):
    ...
    If a line is not identifiably an JUMP_IF_EQ_ACC assembly line, return an
    empty list instead.
