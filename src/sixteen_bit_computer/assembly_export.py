...
def assembly_to_logisim(assembly, default_value=0):
    ...
    Returns:
        (str): String representing contents of the logisim file.
    """
...
def assembly_to_python(assembly):
    """
    ...
    for line in assembly:
        for index, word in enumerate(line.pattern.machinecode):
            ...
            python_line = (
                "        {data: <16} # Line: {line_no:04}{line_part}"
            )
            python_line = python_line.format(
                data=data,
                line_no=line.line_no,
                line_part=line_part
            )
