...
from sixteen_bit_computer.operations import jump_if_positive_flag
...
def test_parse_line(test_input, expected):
    assert jump_if_positive_flag.parse_line(test_input) == expected


@pytest.mark.parametrize("test_input", [
    "JUMP_IF_POSITIVE_FLAG",
    "JUMP_IF_POSITIVE_FLAG A",
    "JUMP_IF_POSITIVE_FLAG A B",
    "JUMP_IF_POSITIVE_FLAG #123 @label",
    "JUMP_IF_POSITIVE_FLAG BLAH",
    "JUMP_IF_POSITIVE_FLAG @l1 @l2",
])
def test_parse_line_raises(test_input):
    with pytest.raises(OperationParsingError):
        jump_if_positive_flag.parse_line(test_input)
